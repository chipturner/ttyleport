...
- **`connect::run()` signature** — takes `ConnectOpts` struct with `destination`, `no_daemon_start`, `ssh_options`. Returns `anyhow::Result<i32>`. Tunnel-only: does not negotiate sessions or run the client relay.
